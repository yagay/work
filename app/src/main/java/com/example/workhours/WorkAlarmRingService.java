...
public class WorkAlarmRingService extends Service {
    ...
    private static final String CHANNEL = "work_alarm_ringing_v3";
    private static final int NOTIFICATION_ID = 7301;
    private static final long VOLUME_KEY_POLL_MS = 120L;
    ...
    private boolean hardwareActionConsumed;
    private boolean ringing;

    private final Runnable volumeKeyMonitor = new Runnable() {
        @Override public void run() {
            if (!ringing || hardwareActionConsumed || audioManager == null) return;
            String action = prefs().getString(
                    WorkAlarmOptions.KEY_ACTION_KEY,
                    WorkAlarmOptions.DEFAULT_KEY_ACTION);
            if (!"stop".equals(action) && !"snooze".equals(action)) return;
            ...
            try {
                ...
                if (lastAlarmVolume >= 0 && current != lastAlarmVolume) {
                    ...
                    handleConfiguredHardwareAction();
                    return;
                }
                lastAlarmVolume = current;
            } catch (Exception ignored) { }
            handler.postDelayed(this, VOLUME_KEY_POLL_MS);
        }
    };
    ...
    @Override public void onCreate() {
        ...
        if (nm != null) {
            ...
            c.setDescription("上班闹钟正在响铃；允许锁屏全屏提醒和持续顶部提醒");
    ...
    @Override public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        currentSnoozeCount = intent == null
                ? 0
                : intent.getIntExtra(EXTRA_SNOOZE_COUNT, 0);
        hardwareActionConsumed = false;
        ringing = true;

        startForeground(NOTIFICATION_ID, buildNotification());
        startHardwareControls();
    ...
    private Notification buildNotification() {
        ...
        int snoozeMinutes = p.getInt(
                WorkAlarmOptions.SNOOZE_MINUTES_KEY,
                WorkAlarmOptions.DEFAULT_SNOOZE_MINUTES);
        boolean canSnooze = p.getBoolean(
                WorkAlarmOptions.SNOOZE_ENABLED_KEY,
                WorkAlarmOptions.DEFAULT_SNOOZE_ENABLED)
                && currentSnoozeCount < p.getInt(
                        WorkAlarmOptions.SNOOZE_MAX_KEY,
                        WorkAlarmOptions.DEFAULT_SNOOZE_MAX);

        PendingIntent full = PendingIntent.getActivity(
                this,
                7302,
                createAlarmScreenIntent(this, currentSnoozeCount),
        ...
        b.addAction(new Notification.Action.Builder(null, "停止闹钟", stop).build());
        return b.build();
    }

    private void startHardwareControls() {
        String action = prefs().getString(
                WorkAlarmOptions.KEY_ACTION_KEY,
                WorkAlarmOptions.DEFAULT_KEY_ACTION);
        if (!"stop".equals(action) && !"snooze".equals(action)) return;
        ...
        try {
            ...
            if (audioManager != null) {
                ...
                handler.postDelayed(volumeKeyMonitor, VOLUME_KEY_POLL_MS);
            }
        } catch (Exception ignored) { }

        try {
            mediaSession = new MediaSession(this, "WorkAlarmHardwareKeys");
                ...
                @Override public boolean onMediaButtonEvent(Intent mediaButtonIntent) {
                    ...
                    if (event != null && event.getAction() == KeyEvent.ACTION_DOWN) {
                        ...
                        if (key == KeyEvent.KEYCODE_HEADSETHOOK
                            ...
                            handler.post(WorkAlarmRingService.this::handleConfiguredHardwareAction);
                            return true;
                        }
                    }
    ...
    private void handleConfiguredHardwareAction() {
        if (hardwareActionConsumed || !ringing) return;
        hardwareActionConsumed = true;
        String action = prefs().getString(
                WorkAlarmOptions.KEY_ACTION_KEY,
                WorkAlarmOptions.DEFAULT_KEY_ACTION);
        if ("stop".equals(action)) stopAlarm();
        else if ("snooze".equals(action)) snooze();
        else hardwareActionConsumed = false;
    }
    ...
    private void startSoundAndVibration() {
        ...
        try {
            ...
            if (ringtone != null) {
                ...
                int fadeSeconds = p.getInt(
                        WorkAlarmOptions.FADE_SECONDS_KEY,
                        WorkAlarmOptions.DEFAULT_FADE_SECONDS);
                if (fadeSeconds > 0) {
        ...
        if (!p.getBoolean(
                WorkAlarmOptions.VIBRATE_KEY,
                WorkAlarmOptions.DEFAULT_VIBRATE)) return;
        try {
            VibratorManager vm = getSystemService(VibratorManager.class);
            vibrator = vm == null ? null : vm.getDefaultVibrator();
            if (vibrator != null && vibrator.hasVibrator()) {
                String pattern = p.getString(
                        WorkAlarmOptions.VIBRATE_PATTERN_KEY,
                        WorkAlarmOptions.DEFAULT_VIBRATE_PATTERN);
                long[] wave;
            ...
            handler.postDelayed(() -> {
                try {
                    if (ringtone != null && ringtone.isPlaying()) {
                        ringtone.setVolume(Math.min(1f, volume));
                    }
                } catch (Exception ignored) { }
            }, i * 1000L);
        }
    ...
    private void scheduleAutoStop() {
        ...
        if (minutes > 0) {
            handler.postDelayed(this::stopAlarm, minutes * 60_000L);
        }
    }

    private void snooze() {
        SharedPreferences p = prefs();
        boolean enabled = p.getBoolean(
                WorkAlarmOptions.SNOOZE_ENABLED_KEY,
                WorkAlarmOptions.DEFAULT_SNOOZE_ENABLED);
        int max = p.getInt(
                WorkAlarmOptions.SNOOZE_MAX_KEY,
    ...
    private void stopAlarm() {
        if (!ringing && ringtone == null && vibrator == null) {
    ...
    private void releaseHardwareControls() {
        try {
