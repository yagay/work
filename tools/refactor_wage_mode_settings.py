...
old = '''        root.addView(text("每小时工资（£）", 14, true));
        ...
        root.addView(rule);
'''
new = '''        hourlySettingsGroup = vertical();
...
old_load = '''        monthlySalaryInput.setText(trimMoney(prefs.getFloat(MONTHLY_SALARY_KEY, 0f)));
    }
'''
new_load = '''        monthlySalaryInput.setText(trimMoney(prefs.getFloat(MONTHLY_SALARY_KEY, 0f)));
...
if old_load not in s:
    raise SystemExit('loadSettings block not found')
